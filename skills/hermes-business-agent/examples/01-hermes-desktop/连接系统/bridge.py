...
def _agent_dir() -> Path:
    """Agent 底座代码目录 = 三系统根下的 Agent系统/（外部 submodule）。"""
    return _project_root() / "Agent系统"


def ensure_syspath() -> None:
    """把三系统目录加入 sys.path（Agent 底座代码在 Agent系统/，不是三系统根）。

    ⚠️ 底座历史上曾平铺在三系统根，`from server import app` 才曾直接可用；
    迁入 Agent系统/ 后必须显式把该目录入 path，否则融合装配静默失败并回退纯业务 app
    （表现为「融合模式」跑起来却没有 Agent 对话路由）。
    """
    base = _project_root()
    for _p in (_agent_dir(), base, base / "业务系统", base / "连接系统"):
        _s = str(_p)
...
def fuse_business_into_agent():
    ...
    """
    ensure_syspath()
    if not (_agent_dir() / "server.py").is_file():
        # 底座不在位时别把「没拉 submodule」伪装成「业务正常但没 Agent」
        print(f"[连接系统] ⛔ Agent 底座缺失：{_agent_dir()}\\server.py 不存在。"
              f"先 `git submodule update --init --recursive` 再启动融合模式。")
        return None
    try:
        # ① Agent 底座（01 根代码）
        ...
        # 降级要刺眼：回退后得到的纯业务 app 没有 Agent 对话路由，
        # 静默回退 = 「融合模式起来了，但里面根本没有 Agent」的假绿源头。
        print(f"[连接系统] ⛔ 融合装配失败，回退业务自建 app（**此 app 无 Agent 对话路由**）："
