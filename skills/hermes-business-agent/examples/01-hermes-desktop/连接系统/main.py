...
# sys.path 的组装唯一由 bridge.ensure_syspath() 负责（含 Agent 底座目录）；
# 此处只把连接系统自身入 path，以便 import bridge。入口不重复维护目录清单，避免两处漂移。
_sys_base = str(Path(__file__).resolve().parent)
...
# start() 拉起第二个 HTTP 服务并卡死（实测 EXE 启动后进程 3→191 指数增长、内存耗尽、
