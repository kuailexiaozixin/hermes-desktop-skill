...
本目录是 `hermes-business-agent` 技能的**可运行参考实现**。每个子目录都是一个独立、可直接运行的
...
| 示例 | 形态 | 技术路线 | 运行期依赖 | 入口 |
|------|------|----------|-----------|------|
| [`01-hermes-desktop/`](01-hermes-desktop/) | 桌面 / 服务 | **进程内集成 Hermes Python Library**，FastHTML 服务端渲染 + pywebview 原生窗口 | `hermes-agent[web]` / `python-fasthtml` / `pywebview` / `uvicorn` 等 | 融合模式 `cd 连接系统 && python main.py`（:8800/dashboard）；纯业务模式 `cd 业务系统 && python app.py`（:8810/dashboard）。`Agent系统/` 是 git submodule，克隆后须先 `git submodule update --init --recursive`，否则两种模式都起不来 |
