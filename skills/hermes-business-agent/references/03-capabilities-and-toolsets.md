...
- **code_execution** (1) — Run Python scripts that call tools programmatically（减少 LLM 往返）· `execute_code`
  **实测口径（0.19.0 / Windows 10，2026-09-22）**——先记一条踩坑：模块 docstring 自陈
  "Platform: Linux / macOS only … **Disabled on Windows**"、`check_sandbox_requirements()` 的 docstring 自陈
  "requires a POSIX OS for Unix domain sockets"，**两句都与实现相反**：实际 `SANDBOX_AVAILABLE = True`，
  且 Windows 分支 `_use_tcp_rpc = _IS_WINDOWS` 把本地 RPC 从 Unix domain socket 换成 loopback TCP。
  本机跑通两次：`execute_code(code="print(...)")` → `status success / exit_code 0`；
  脚本内 `hermes_tools.read_file(...)`（`enabled_tools=["read_file"]`）→ `tool_calls_made: 1` 并取回真实内容。
  ⇒ **桌面路线 ① 可用**，禁止按 docstring 判它不可用（能力可用性一律以代码 + 实测为准，R6）。三条**真实**边界（都不是 Windows）：
  ① 沙箱可调工具是**固定 7 白名单** `SANDBOX_ALLOWED_TOOLS`（`web_search` `web_extract` `read_file` `write_file`
  `search_files` `patch` `terminal`），再与本轮已启用工具取交集 ⇒ **里面没有任何业务工具**，它折的是内核文件/检索/网页链，不是业务链；
  ② 只有脚本 stdout 回模型，**中间工具结果永不进上下文**——这既是它省往返的机制，也是审计盲区（断言只能落在 stdout，见 `21` §6）；
  ③ 审批粒度是**整段脚本一次批**（`tools.approval.check_execute_code_guard` 自陈 "approval is one-shot for this run"），
  且其 docstring 记明"纯本地 / 非交互 / 无 TTY / 非 gateway 会话直接返回 approved"——桌面进程内形态默认正落在这个范围里（红线 `07` R13）。
  上限可配：`config.yaml → code_execution.*`（默认 `timeout` 300s、`max_tool_calls` 50、stdout 上限 50 KB）。
  轮次与 token 的账怎么算归 `21` §6，业务侧该不该用归 `19` §3 归属三问。
- **delegation** (1) — Spawn subagents with isolated context for complex subtasks · `delegate_task`
  注意：委派事件的 SSE 透传未实测（R6）。
...
- **kanban** (12) — Kanban 多智能体协调；仅当 Agent 被 kanban dispatcher 拉起时激活（`HERMES_KANBAN_TASK` 环境变量）；dispatcher 默认在网关内运行（见 `kanban.dispatch_in_gateway`）·
  `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`,
  `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`
...
### 4.1 复合/元工具集（4 个）
- **hermes-cli** (52) — Full interactive CLI toolset：全部默认工具 + cronjob 管理 ·
  基础 52 工具 = `web/file/browser/vision/skills/tts/todo/memory/session_search/clarify/
  execute_code/delegate/cronjob/ha_*/kanban/computer_use` + `terminal/process/read_terminal/close_terminal`
- **hermes-cron** (52) — Default cron toolset：与 hermes-cli 同核心工具，受 `hermes tools` 门控
- **hermes-api-server** (35) — 见 §3.8（OpenAI 兼容 HTTP 用，无交互 UI 工具）
- **hermes-acp** (29) — 见 §3.8（编辑器集成）

### 4.2 消息平台（20 个，未标数字的行 = 与 hermes-cli 同为 52，差异在后）
| 工具集 | 差异（在 52 基础工具上额外/特有） | 说明 |
| --- | --- | --- |
...
| `hermes-discord` (54) | + `discord`, `discord_admin` | Discord bot（terminal 经危险命令审批） |
| `hermes-feishu` (57) | + `feishu_doc_read`, `feishu_drive_*` | 飞书/Lark 企业消息 |
| `hermes-yuanbao` (57) | + `yb_*` | 元宝消息平台 |
...
三行的 `enabled_toolsets` 全是 `None`，这不是巧合，是本节唯一要教的形状：**只用减法**。

| 需求 | `disabled_toolsets`（`enabled_toolsets` 恒为 `None`） |
| --- | --- |
| 桌面起点：保留联网/文件/记忆，去掉命令执行 | `["terminal"]` |
| 离线：进程不得发出任何网络请求 | `["terminal", "web", "search", "x_search", "browser", "vision", "video", "image_gen", "video_gen"]`，接了消息平台或 MCP 的再按 §3.7 与 §4 的名字逐个追加 |
| 只留某几类能力（如只要文件与检索） | 把**其余**逐个列全——清单必须穷尽，漏一个就是一个出网面 |

> 为什么不许写 `enabled_toolsets=["file"]`（R4）：那一句会把未列出的联网/记忆/浏览器全部砍掉，
> 而且砍了什么从参数上看不出来。内核确有 `coding` / `safe` / `debugging` 这类聚合工具集（§3.8），
> 但按名放行它们就得显式填 `enabled_toolsets`，正撞这条红线；要参照它们的成员范围，
> 去 §3.8 抄成员清单、翻译成上面第三行那种穷尽减法，不要把聚合集名填进 `enabled_toolsets`。
...
| `hermes-*` 消息平台（20 个） | 聚合变体 | 52/54/57 工具变体，模块为 `hermes-cli` 并集 + 平台特有（见 §4.2） |
