...
> 标「禁止」的能力一般不在进程内直跑路线启用；注明「数据落 `HERMES_HOME`」的能力，其打包态路径 = 启动器钉住的根
> （本实现钉在 `<exe>/hermes_data`，钉法与成因见 `05` §3）。`05` §3 是这条事实的唯一来源，本文各节一律引用它、不重述。
...
> LLM Wiki 类需求应在**应用层自建**——内置参考实现 `examples/01-hermes-desktop/Agent系统/wiki_engine.py`
...
- **反模式红线**：禁止把 Goals 当成主模型的系统提示硬塞进每轮 prompt；它应通过"回合末判定"闭环
  （`evaluate_after_turn` 语义），而非注入主对话上下文。

### 1.1 进程内集成实战（基于 `examples/01-hermes-desktop/Agent系统/hermes_features/` 真实写法）
...
#     注意：该判定会消耗一次辅助模型调用——须先用 get_text_auxiliary_client("goal_judge")
...
- **进程内集成范式**：构造 `AIAgent` 时开 `checkpoints_enabled=True` 及相关 `checkpoint_max_snapshots` /
  ...
  核实）。桌面 UI 把"快照列表 + 回滚"做成面板；快照存储落在 `HERMES_HOME` 内（冻结态即启动器钉住的那个根，`05` §3）。
- **反模式红线**：禁止自己手写 sqlite / 文件拷贝来实现"快照"——复用内核 `CheckpointManager`，
  否则会丢失 shadow-git 去重与自动裁剪语义。

### 2.1 概念澄清：内核 Checkpoint（文件快照）vs 会话消息快照（应用层）

**易混点**：Hermes 有两套"快照"，**不要混为一一**：

| 维度 | 内核 Checkpoint（`tools.checkpoint_manager`） | 会话消息快照（应用层，如 examples/01-hermes-desktop） |
| --- | --- | --- |
...
消息快照（examples/01-hermes-desktop 的 `checkpoints_*` 即此模式，落盘在 `HERMES_HOME/checkpoints/`）。
...
> 注意：examples/01-hermes-desktop 的 `checkpoints_*` 是**消息快照**（落 `HERMES_HOME/checkpoints/<cid>/`），
...
- **进程内集成范式**：桌面 UI 提供 MoA 预设编辑器（`resolve_moa_preset` / `set_active_moa_preset`），聊天界面用
  `tool_progress_callback`（见 `02` §3）驱动"参考模型"折叠块渲染；用户消息前加 `/moa` 即触发。
- **反模式红线**：禁止试图把 MoA 做成 `AIAgent` 的一个"模型工具"——它本质是回合标记 + 聚合上下文，
  由 `aggregate_moa_context` 在 `agent.moa_loop` 内完成，不是工具调用。

### 3.1 进程内集成实战（基于 `examples/01-hermes-desktop/Agent系统/hermes_features/` 真实写法）
...
- **反模式红线**：禁止用会话 `cwd` + git probe 去"推断"工作区（旧桌面实现踩过的坑）——项目是显式一等存储，
  用 `projects_db` 的 API 读写，不要自建推断逻辑。

### 4.1 进程内集成实战（基于 `examples/01-hermes-desktop/Agent系统/hermes_features/` 真实写法）
...
- **反模式红线**：禁止把捆绑包当成"新对象类型"去建独立存储——它只是 YAML + 斜杠命令别名，复用
  `agent.skill_bundles` 即可。

### 5.1 进程内集成实战（基于 `examples/01-hermes-desktop/Agent系统/hermes_features/` 真实写法）
...
- **反模式红线**：禁止把审计做成阻塞启动的硬门禁——`security_audit_startup` 的设计语义就是
...
- **反模式红线**：禁止把蓝图当成独立调度对象去自研——它底层就是 skill + cron，复用 `tools.blueprints`
...
- **反模式红线**：禁止在桌面主线程里串行跑数据集——`BatchRunner` 用 `multiprocessing.Pool`
...
- **反模式红线**：禁止把 Journey 当成独立数据库去自建——它是记忆/技能学习记录的时间线投影，
  复用现有记忆与策展状态即可。
...
- **内核模块**：`hermes_cli.backup`（docstring 已核实），数据落 `HERMES_HOME`。
...
- **进程内集成范式**：桌面 UI 提供"备份 / 还原"面板；备份须捕获**启动时实际解析出的那个根**
  （打印 `hc.get_hermes_home()` 得到，不要写死 `<exe>/hermes_data`——外层设过 `HERMES_HOME` 就不是它），
  还原前按 `07` §运行数据保护规则先备份→变更→还原→md5 校验。
- **反模式红线**：禁止还原时直接覆盖运行中的出厂数据——遵循"先备份、再变更、再还原、md5 校验"铁律（见 `07`）。
...
- **进程内集成范式**：桌面 UI 提供多 profile 切换器。桌面单目录交付通常只钉一棵根（`05` §3），
  所以多 profile 隔离优先用该根下的 profile 子目录。要按 profile 换整棵根并非不可——启动器用的是
  `setdefault`，外层先设 `HERMES_HOME` 即让位，或走 `hermes_constants` 的覆盖 API；代价是每棵根都要各自备份、
  各自清理，卸载时不再"删目录即删干净"。
- **反模式红线**：禁止把外部 `hermes profiles` 的"每个 profile 一棵独立 `HERMES_HOME`"语义原样搬进打包态而不改卸载与备份逻辑。
  打包态的根是启动器 `setdefault` 钉的，"删制品目录即删干净"只在**只有一棵根**时成立；分了多棵，
  备份、清理、升级回滚都要按棵各做一遍（`05` §3）。
...
- **反模式红线**：禁止在主线程/UI 线程同步跑策展——它是后台辅助模型任务，须放后台，避免阻塞对话。
...
- **反模式红线**：禁止在桌面硬编码供应商清单或 base_url——`hermes_cli.providers` 是合并了 models.dev
...
- **动态查询示例（实测 0.19.0，uv 环境）**——桌面 Settings 页应这样用 `provider_catalog()` 渲染，而非硬编码：
  ...
  d = by_slug["anthropic"]           # 注意：必须运行时查——openai/ollama 等并无固定 slug，硬编码会失效
  d.auth_type                        # 'api_key'
...
- **反模式红线**：禁止自建 `tasks` 表去复制看板状态——`kanban` 工具集已封装底层存储与状态机，
