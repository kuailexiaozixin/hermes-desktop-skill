...
> 本文把「怎么确认一个 Hermes 集成**真的跑通了**」串成一段可执行的 walkthrough，
> 并特别考量 **Hermes 是一个 Agent 智能体**带来的测试特殊性——它不是无状态 REST 端点，
> 「测试通过」不等于「HTTP 200」，而是「Agent 真正用事件流把任务跑到了 `done`」。
>
> **适用路线**：全路线。Walkthrough 以**路线①（进程内直跑）**为示范写（桥接链最短、事件看得最清）；
> 选 ②/④/⑤ 时 Step 2–3 的"接事件总线"换成对 `15` §4bis 审批协议与 SSE 端点的订阅、Step 4 断言按 `15` §10 检查清单补端点项，
> 选 ③ 时事件流只能从子进程 stdout 解析，A1–A9 里依赖细粒度回调的断言需改判。
>
> **主线位置**：本文是 SKILL.md **第 ⑦ 步（取证）** 里 Test 段的展开细则，本身不定义流程顺序。Step 与主线的对应：
>
> | 本文 | SKILL.md 主线 |
> | --- | --- |
> | Step 0 前置核查 | 入场检查（环境可跑性） |
> | Step 1 `runtime_ready()` | 第 ⑥ 步（自检）／`07` §3 |
> | Step 2–3 构造 + 收事件流 | 第 ③④ 步 |
> | Step 4 专项断言 A1–A9 | 第 ⑦ 步 Test 第 3 项（同一次真实对话） |
> | Step 5 / 5b 场景任务与宿主原有功能用例 | 第 ⑦ 步 Test 第 3 项（同帧取证） |
> | Step 6 接入门禁 | 第 ⑧ 步（`release_gate`） |
> | §8 输出评估 / §9 离线回放 | 第 ⑦ 步 Test 第 3–4 项 + `20` §4.4 各段的运行位置 |
>
> 业务侧用例的派生方法（规模、形状、归因）不在本文，见 [`20-output-contract-and-golden-set.md`](20-output-contract-and-golden-set.md) §4。
...
| **进程内直跑、无网关** | 选用进程内直跑路线时不起 Hermes 网关、不 spawn `hermes` 子进程、不连 `127.0.0.1:8642`（`07` R1/R2）。测试要 statically 断言「没起网关、没拉起 hermes 子进程」（改选跨进程路线则按对应路线手册另测，见 references/02-integration-core.md §2.1–§2.5（五条路线逐节））。 |
| **异步 / 事件驱动** | 流式靠后台 `worker` 线程 + `queue.Queue`（`02` §3、§6）。测试不能在 UI 主线程同步跑，要像生产代码一样用 worker + queue 收集事件。 |
...
| **委派事件未证实** | `event_callback` 是否透传子代理委派事件**未经实测**（R6）。测试若依赖委派卡片，必须先实测透传，否则按「静默不显示」处理，不得假设有 `delegation` 事件。 |
...
生产代码用 worker 线程 + queue（`02` §3、§6）。自测保持一致，避免在调用线程里阻塞。
...
L2 通过后，回到 SKILL.md 第 ⑧ 步继续（脚本↔步骤映射见 07 §4）：
...
| A5 | `action` 数 == `action_result` 数 | 工具闭环 | 工具调了没回结果（阻塞无响应或异常） |
...
## 6. 测试反模式（以下这几条会制造假绿） <a id="anti-patterns"></a>
...
# -*- coding: utf-8 -*-
"""
self_test_integration.py — 业务系统进程内集成自测（跑通一个集成）
...
> 旗舰示例 `examples/01-hermes-desktop/Agent系统/agent_runtime/` 的 `runtime_ready()` /
...
> hermes 无内置 test model，但可**用固定回放（fixture）让 Agent 走确定性路径**，
> 使 CI 在**无 provider key / 无网络**下仍能验证集成链路与事件形状。旗舰示例 `examples/01-hermes-desktop/Agent系统/test_bridge.py`
