...
进程内直跑路线的核心约束（见 `07` §1、`10` §1）：若选用**进程内直跑**路线，一般**不起网关、不 spawn 子进程、不开 API Server**（跨进程路线的设施按对应手册按需启用，选型与落地见 references/02-integration-core.md §2.1–§2.5（五条路线逐节））。
...
> 注：`top_level.txt` 顶层模块共 23 个（见 `00-index.md` §4 事实基线），其中 `run_agent`/`tools`/`agent`/`hermes_cli`/`batch_runner` 及 `11` 覆盖的支撑模块已在 `01`/`10`/`12`/`13`/`11` 详述；
...
- **若要在进程内调用外部 MCP 工具**：用 `tools.mcp_tool`（`12` 已列 `discover_mcp_tools`，另有 `MCPServerTask` / `reconnect_mcp_server`），而非起这个 server。
...
> 其余模块按「进程内直跑」取舍：网关/CLI/调度/协议桥等进程外设施，进程内直跑时一般不起、不 spawn（改选跨进程路线则按需启用，选型见 references/02-integration-core.md §2.1–§2.5（五条路线逐节））。
