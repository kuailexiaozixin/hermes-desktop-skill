...
> `gateway/platforms/api_server.py`（5565 行，0.19.0 核实）。
> **定位**：本文是「API Server / `/v1`」路线的**完整落地手册**（何时选 / 怎么落地 / 与进程内直跑对照自建）；
> 唯**服务侧运维**（守护、日志轮转、滚动升级与回滚）不在本文，归 `06` §9。
> 呼应入口：`02` §2.4–§2.5（路线④/⑤ 的判据、拓扑、代价）、`04` §16（何时放开判据）、`10` §2.5（模块存在性）、
> `06` §9（方式 A 作为**常驻服务**的部署、守护、日志、滚动升级与回滚——本文讲能力与配置，那节讲怎么把它跑在服务上不塌）。
...
| 需求 | 是否选 API Server |
| --- | --- |
| 外部 OpenAI 兼容前端（Open WebUI / LobeChat / LibreChat / ChatBox…）要当客户端调 | 应选 |
| curl / CI / 其它编程语言要调用 Hermes（语言无关 / 非 Python 消费） | 应选 |
| 接消息平台（Telegram/Slack/QQ/Discord…）或 cron 多投递 | 应选（网关形态，非纯 API Server） |
| 多客户端 / 远程 / 多用户 profile 隔离 | 应选 |
| 需要 runs/jobs/sessions 等完整管理端点 | 应选（网关 API Server，最全） |
| **纯桌面单机 GUI、仅进程内自调、无对外被调需求** | 不选（保持进程内，避免常驻服务/端口/认证/跨进程状态开销） |

呼应：`02` §2.4–§2.5、`04` §16「何时放开」判据。
...
>    交付前直接走总闸：`python scripts/release_gate.py --verify-launch --api-server --api-key <key>`
>    （服务未起时该步以退出码 2 记 SKIP，不阻断进程内形态；起着但断言不过 = 硬失败）。
...
## 4ter. 身份绑定调用链与凭证生命周期（0.19.0 逐站实测）

> 上一节的两个会话头**流向的是"作用域标识"，不是"凭据"**。这一节把一条链走完，标出它在第几站断掉——
> 断点决定了哪些东西**必须**由业务侧实现（`19` §5），而不是"配一下就能安全"。

| 站 | 实测事实（0.19.0，`api_server.py` 除注明外） | 对集成的含义 |
| --- | --- | --- |
| ① 入口认证 | `_check_auth()` 只认**一把静态 Bearer key**（`:1235-1265`，`hmac.compare_digest`，源 `extra.key` 或 `API_SERVER_KEY`，`:948`）；失败 401 + 审计日志 | 无 per-user 账号 / 角色 / scope / ACL / 按身份配额（`role` 一词全是 LLM 消息角色）。"谁能做什么"**不可能由网关给出** |
| ② 会话头 | 调用方可提供的身份信息只有 `X-Hermes-Session-Id` / `X-Hermes-Session-Key` 两个（`:2724-2762`、`:1539-1589`）；未配 key 时带它们直接 403；控制字符 / 路径穿越形状 / >256 字符 → 400 | 校验的是**形状**，不是**归属**——头值由客户端自报，服务端不认识它属于谁 |
...
启动即强校验（无 key / 弱 key 拒绝启动，`:5355-5372`）；`.env` 落盘方式与"不进制品"的约束见 `06` §9.5；
`model_routes` 里那些 `api_key` 是**上游供应商**的 key，与调用方凭据无关（`:965`、`:1679-1698`），禁止混在同一张密钥清单里管理。
另有一处例外要知道：`POST /api/cron/fire` 用 NAS 签发的 JWT 而非 `API_SERVER_KEY`（`:1521-1523`、`:4349`）。

**文件作用域：链上最弱的一环，别当边界用。** 写侧是黑名单 + 一个**默认关闭**的 env 白名单：

- 黑名单 `agent/file_safety.py`（`build_write_denied_paths/prefixes` `:28-77`，判定 `:98-155`）：
  `~/.ssh`、`.aws`、`.gnupg`、`.kube`、`.docker`、`.config/gh`、`.env`、`.netrc`、`/etc/sudoers*` 等，
  外加 Hermes 自有状态 `state.db`、`sessions/`、`mcp-tokens/`、`pairing/`（`:120-143`）。
- **唯一的白名单开关是环境变量 `HERMES_WRITE_SAFE_ROOT`**（`:80-95`，`os.pathsep` 分隔；注意：不是 `config.yaml` 键），
  未设 = 该特性完全关闭、全盘无根限制。
...
> 业务主体、权限、行级范围、按人配额**全部在链外**，须由连接层或工具 handler 自己实现
> （`19` §5 的 `current_scope()` 是唯一被认可的落点）。
...
- 复用 `01` §3 构造 + `02` §3 worker+queue 事件分发；安全护栏走 `02` §7。
...
| 部署 | 无需运维 | A 要守护常驻服务（守护/日志/升级与回滚的落地见 `06` §9） |
| 序列化 | 无 | HTTP 开销 |
...
- [ ] 已选对方式（A 官方网关 / B 进程内自建 / C 仅 dashboard），并在 `02` §2.4–§2.5 明确"为何放开"。
...
**方式 A 部署为常驻服务时，再加三条（落地做法唯一归 `06` §9）**：

- [ ] 守护层已配（服务包装器 / systemd），首启预检不过即带原因退出；日志已重定向到文件并轮转。
- [ ] 手动 kill 后**重启自恢复**、`/health/detailed` 回到就绪；两客户端各带自己的会话头并发对话不串台。
- [ ] 在预发环境走完滚动升级六步**并执行过一次回滚**（制品回滚与数据回滚可分别执行）。
