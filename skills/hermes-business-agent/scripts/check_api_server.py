...
⚠️ 说明：
  ...
  * 这是「API Server 形态」的落地校验，**不阻塞**进程内默认路线（进程内形态本就不开 8642）。
    加 `--skip` 后，连不上服务时以退出码 2（SKIP）返回，供上层门禁（release_gate）判读为「不适用」。
  * 用标准库 urllib，无第三方依赖。

退出码：0 = 全部通过（health 200 + 认证存在 [+ models 200]）；1 = 某项失败（阻断）；
        2 = 参数/使用问题，或服务未运行且指定了 --skip（视为跳过）。
"""
...
def main() -> int:
    ...
    if st == -1 and args.skip:
        # 连接失败 = 服务没起。进程内形态本就不开 8642，属「不适用」而非失败。
        print(f"[SKIP] {base} 不可达（{body}）——未启用 API Server，本项不适用")
        return 2
    if st == 200:
    ...
    if not ok:
        # 到达了服务但有失败项 —— 这是真失败，不因 --skip 而豁免（--skip 只豁免「服务不可达」）
        print("\n结果：存在失败项（API Server 已应答但断言未过）。")
        return 1
