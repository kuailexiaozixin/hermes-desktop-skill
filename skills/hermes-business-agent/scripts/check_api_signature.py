...
def locate_run_agent(explicit: str | None = None) -> str | None:
    ...
    # ② Windows: %LOCALAPPDATA%/hermes-business-agent/venvs/*/
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    if _local_appdata:
        _venvs_dir = os.path.join(_local_appdata, "hermes-business-agent", "venvs")
...
def fetch_upstream_run_agent() -> tuple[str | None, dict | None, str | None]:
    ...
    _UA = "hermes-business-agent-skill/1.0"
    try:
    ...
    if args.json:
        ...
        if report["OK"]:
            ...
            print(
                ...
                "   按 SKILL.md〔入场检查〕更新 references/01-library-api.md 与 examples，再宣称「已适配」。"
            )
