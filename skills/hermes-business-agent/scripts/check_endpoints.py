...
# examples/01 是三系统根；可运行底座（main.py + static/）在其下的 Agent系统/（外部 submodule）
DEFAULT_EXAMPLE = os.path.join(SKILL_ROOT, "examples", "01-hermes-desktop", "Agent系统")
DEFAULT_MAIN = os.path.join(DEFAULT_EXAMPLE, "main.py")
DEFAULT_STATIC = os.path.join(DEFAULT_EXAMPLE, "static")
...
def main(argv=None) -> int:
    ...
    p.add_argument("example_dir", nargs="?", default=None,
                   help="示例目录（含 main.py 与 static/app.js）；默认 examples/01-hermes-desktop/Agent系统")
    ...
    if args.js:
        js_files = [args.js]
        static_dir = os.path.dirname(args.js)
    else:
