...
# -*- coding: utf-8 -*-
"""
quality_check.py — hermes-business-agent 全量质量门禁编排（SKILL.md 第 ⑦ 步「取证」；细节见 references/07-quality-gates.md）

把分散的门禁合成一条命令，CI / 提交前 / 构建前跑一次即可：
  [1] 语法编译       —— 递归 py_compile 所有 .py（agent_runtime / _testkit / test_bridge / 各脚本）
  [2] 技能结构门禁   —— check_skill_gate.py（主线引用文件齐备 + 无未登记孤儿素材 + hermes-llms-full.txt 在位）
...
# examples/01 是三系统根；Agent 底座代码在其下的 Agent系统/（外部 submodule，可能未初始化）。
TRI_ROOT = os.path.join(SKILL_ROOT, "examples", "01-hermes-desktop")
AGENT_DIR = os.path.join(TRI_ROOT, "Agent系统")
EXAMPLE_DIR = AGENT_DIR
PY = sys.executable
...
def step_bridge() -> tuple[bool, str, str]:
    """离线桥接测试（底座 test_bridge.py）。

    Agent系统/ 是外部 submodule：未初始化时不是代码坏了，而是底座不在位 → 降级 skip，
    并给出取回底座的命令（真·失败仍返回 fail 阻塞）。
    """
    tb = os.path.join(AGENT_DIR, "test_bridge.py")
    if not os.path.isfile(tb):
        return True, (f"SKIP：未找到 {tb}\n"
                      "        底座 submodule 未初始化？跑 `git submodule update --init --recursive`"), "skip"
    ok, out, _rc = _run_step("bridge", ["test_bridge.py"], cwd=AGENT_DIR)
...
def main() -> int:
    ...
    # [3] bridge
    p, o, kind = step_bridge()
    results.append(("离线桥接测试 (test_bridge)", p, o, kind))
    ...
    # 输出
    print("=" * 64)
    print(" hermes-business-agent 全量质量门禁")
    print("=" * 64)
