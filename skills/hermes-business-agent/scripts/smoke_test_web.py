#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""hermes-business-agent 网页回归测试夹具（无头、离线、可重复、可扩展）。
...
# examples/01 是三系统根；可运行的底座 app 在其下的 Agent系统/（外部 submodule）
DEFAULT_ROOT = SKILL_ROOT / "examples" / "01-hermes-desktop" / "Agent系统"
...
def main(argv=None) -> int:
    ...
    if failed:
        return 1
    print("  （本夹具只覆盖结构级；真实 LLM 往返见 SKILL.md 第 ⑦ 步（取证），端到端 walkthrough 见 references/09-integration-e2e.md）")
    return 0
...
if __name__ == "__main__":
    ...
    # os._exit 跳过解释器收尾 → 必须先手动 flush，否则重定向/管道下 stdout 缓冲里的
    # JSON 结果会被整体丢弃，上层（quality_check）读到空输出即误判失败。
    sys.stdout.flush()
    sys.stderr.flush()
    os._exit(_code)
