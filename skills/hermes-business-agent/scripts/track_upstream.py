...
# -*- coding: utf-8 -*-
"""
track_upstream.py — Hermes 上游漂移跟踪（SKILL.md〔入场检查〕「工具链没漂、语义有源」，三条跟踪线）。
...
# ---- 基线常量（版本单一事实来源 = scripts/api-baseline.json；改了要同步 SKILL.md〔入场检查〕与 01-library-api.md）----
PKG = "hermes-agent"
...
def _http_get(url: str, binary: bool = False):
    req = urllib.request.Request(url, headers={"User-Agent": "hermes-business-agent-skill/1.0"})
...
def main(argv=None) -> int:
    ...
    if args.json:
        ...
        if not args.gate:
            ...
            print("=" * 60)
            if drift:
                print("⚠️ 存在破坏性漂移：先按 SKILL.md〔入场检查〕更新技能，禁止带病作业。")
            else:
                print("✅ 无破坏性漂移。")
