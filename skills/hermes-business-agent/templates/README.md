...
## 升级路径（对应 SKILL.md 第 ③→④→⑤ 步；打包交付在第 ⑧ 步）

空壳跑通后，把 `stream_worker`/`worker` 升级为
`examples/01-hermes-desktop/Agent系统/agent_runtime/`（`_chat.py` / `_tools.py` / `_toolsets.py`）
的完整版（加工具卡片、思考折叠区、会话历史、审批），再注册业务工具（`references/02-integration-core.md` §7.1）、
接表格办公（同文件 §7.3），最后打包（`references/06-packaging.md`）。
