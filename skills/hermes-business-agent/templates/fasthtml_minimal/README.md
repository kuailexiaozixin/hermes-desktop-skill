...
## 下一步（按 SKILL.md 主线的第 ③→④→⑤→⑧ 步）

1. `scripts/probe_library.py` 确认 Library 健康。
2. 把 `stream_worker` 升级成 `examples/01-hermes-desktop/Agent系统/agent_runtime/_chat.py` 的完整版
   （加 `tool_start/complete`、`reasoning` 回调 + `_ThinkingSplitter`）。
3. 注册业务工具（`references/02-integration-core.md` §7.1 三层范式；面按第 ① 步契约表的面表给）。
4. 表格/办公需求走 `references/02-integration-core.md` §7.3 治理模型。
